...
Product names, category placement, suggested retail prices, and images were derived from the provided `VIDVIE_UAE_Full_Catalog (6).pdf`. The B2B wholesale unit prices and the source PDF are excluded from the public repository. The deployed site contains only public product data in `catalog.js` and optimized product images in `assets/catalog/`. Brand background was checked against [VIDVIE's global site](https://www.vidvie.hk/about/index.html).
